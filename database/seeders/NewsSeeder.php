...
class NewsSeeder extends Seeder
{
     ...
     *
     * Menyalin gambar thumbnail dari public/uploads/news/ atau public/assets/img/
     * ke storage/app/public/news/ sesuai konvensi NewsController.
     * Nama file thumbnail di kolom DB mengikuti nama file asli dari uploads.
     */
    public function run(): void
    {
        $newsData = [
            [
                ...
                // Sumber: tidak ada di uploads/news, gunakan gambar lomba1.jpg dari assets sebagai pengganti
                '_source'    => public_path('assets/img/lomba1.jpg'),
                'status'     => 'publish',
            ...
            [
                ...
                // Sumber: tidak ada di uploads/news, gunakan DSC09220 sebagai pengganti bertema kunjungan
                '_source'    => public_path('uploads/news/DSC09220-768x432.jpg'),
                'status'     => 'publish',
            ...
            [
                ...
                // Sumber: tersedia langsung di public/uploads/news/
                '_source'    => public_path('uploads/news/LPK-One-Creative-2-768x432.jpg'),
                'status'     => 'publish',
        ...
        // Fallback image: gunakan Gedung.png dari assets sebagai gambar default
        $fallbackSource = public_path('assets/img/Gedung.png');

        foreach ($newsData as $item) {
            $storagePath = 'news/' . $item['thumbnail'];

            // Salin gambar ke storage jika belum ada
            if (!Storage::disk('public')->exists($storagePath)) {
                $source = $item['_source'] ?? null;

                if ($source && file_exists($source)) {
                    // Salin dari sumber yang ditentukan (uploads/news atau assets/img)
                    Storage::disk('public')->put($storagePath, file_get_contents($source));
                } elseif (file_exists($fallbackSource)) {
                    // Fallback ke Gedung.png jika sumber tidak ditemukan
                    Storage::disk('public')->put($storagePath, file_get_contents($fallbackSource));
                } else {
                    // Last-resort: dummy 1x1 pixel JPEG
        ...
        // Hapus kunci _source sebelum insert ke database
