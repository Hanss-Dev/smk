      ...
      <div class="card">
            ...
            <tbody>
              @forelse($highlights as $i => $h)
                <tr>
                  ...
                  <td>
                    @if($h->image)
                      <img class="preview-image" src="{{ asset('storage/highlight/' . $h->image) }}" style="height:50px; object-fit:cover; border-radius:4px;">
                    @else
        ...
        <div class="card-footer clearfix">
          <div class="float-right">
            {{ $highlights->appends(['search' => request('search'), 'per_page' => request('per_page')])->links() }}
          </div>
        </div>
      </div>
